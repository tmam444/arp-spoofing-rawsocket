...
- **IPv6 NDP (Neighbor Discovery Protocol) Spoofing** 지원
- **Target ↔ Gateway**의 ARP/NDP 캐시를 모두 오염시켜 완전한 MITM 환경 구축
- **Raw Socket** 기반 이더넷 프레임 및 ARP/NDP 패킷 수동 생성
- **패킷 저장 기능**: MITM 이후 수신한 패킷을 PCAP 형식으로 저장
- **IPv4 및 IPv6 대응**, 네트워크 인터페이스 자동 선택 및 MAC 주소 파싱 처리
...
- Protocols: **Ethernet, ARP, IPv6 NDP (ICMPv6)**
- Libraries: `<netinet/if_ether.h>`, `<sys/socket.h>`, `<net/if.h>`, `<netinet/icmp6.h>`, etc.
...
# IPv6 NDP Spoofing
sudo ./malcolm 2001:db8::10 aa:bb:cc:dd:ee:ff 2001:db8::20 11:22:33:44:55:66
```
...
| `main.c` | 인자 파싱, 인터페이스 선택, 소켓 생성, ARP/NDP 패킷 생성, 전체 흐름 제어 |
| `spoof_gateway.c` | ARP/NDP 패킷 생성, 전송 관련 함수, IPv4/IPv6 게이트웨이 스푸핑 |
| `pcap.c` | PCAP 헤더 정의 및 패킷 저장 기능 |
| `utils.c` | MAC/IP 출력, 에러 처리, IPv4/IPv6 주소 출력 등 유틸리티 함수 |
| `interface.c` | 네트워크 인터페이스 선택 (IPv4/IPv6 지원) |
...
- IPv6 NDP (Neighbor Discovery Protocol) 구조와 동작 원리
- ICMPv6 패킷 구조 및 체크섬 계산
- 네트워크 인터페이스 및 MAC/IP 주소 파싱 (IPv4/IPv6)
...
- ~~IPv6 NDP Spoofing 추가~~ ✅ **완료**
- IPv6 NDP Router Advertisement/Solicitation 지원 확장
- ICMPv6 체크섬 계산 최적화
